...
- There is no dedicated lint config in `pyproject.toml`.
- There is no checked-in pytest suite right now.
...
There is no formal `build` script, no formal lint task, and no checked-in pytest suite.
...
- Health check after server start: `curl http://127.0.0.1:8000/health`
- Current pytest status: `uv run python -m pytest`
- Expected result today: this fails because `pytest` is not installed and there is no test suite yet.
- Single-file test command once pytest exists: `uv run python -m pytest tests/test_file.py -q`
- Single-test function pattern once pytest exists: `uv run python -m pytest tests/test_file.py -k test_name -q`
...
- Use type hints everywhere practical; the codebase prefers built-in generics like `list[dict]` and unions like `str | None`.
...
- When adding tests later, prefer lightweight pytest coverage around helper modules before full end-to-end coverage.
